...
const RecipeView = class extends View {
  ...
  addHandlerRender(handler) {
    // Call the same function for multiple events (page "load" and id change in the url ("hashchange"))
  ...
  addHandlerUpdateServings(handler) {
    this._parentElement.addEventListener("click", function (eve) {
      const btn = eve.target.closest(".btn--update-servings");
      if (!btn) return;
  ...
  addHandlerAddBookmark(handler) {
    this._parentElement.addEventListener("click", function (eve) {
      const btn = eve.target.closest(".btn--bookmark");
      if (!btn) return;
      handler();
    });
  }
